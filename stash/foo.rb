...
#                       instructions: [ apply_ins ] )
  ...
  def packet_in datapath_id, message
    ...
    if @state == -1
      send_message datapath_id, GetConfigRequest.new( transaction_id: 2222 )
    end
    if @state == -1
      send_message datapath_id, EchoRequest.new( transaction_id: 2222, user_data: "This is an echo test".bytes )
      #send_message datapath_id, EchoRequest.new( transaction_id: 2222 )
    end
    if @state == -1
       match = Match.new( in_port: 1, eth_type: 2054  )
...
#      send_barrier_request datapath_id
  ...
  def echo_reply datapath_id, message
    puts message.inspect
    unless message.user_data.nil?
      str = ""
      message.user_data.each { | e | str << e.chr }
      puts str
    end
  end
