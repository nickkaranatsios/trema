...
write_message_buffer( message_buffer *buf, const void *data, size_t len ) {
  ...
  if ( ( buf->head_offset + buf->data_length + len ) <= buf->size ) {
    ...
    buf->head_offset = 0;
    if ( buf->data_length + len <= buf->size ) {
      memcpy( ( char * ) buf->buffer + buf->data_length, data, len );
    }
    else {
     error( "exceeding buffer size %d %d", buf->data_length, len );
    }
  }
  buf->data_length += len;
...
on_send_write( int fd, void *data ) {
  ...
  while ( ( send_len = get_send_data( sq, sent_total ) ) > 0 ) {
    ...
    writecb.aio_nbytes = send_len;
    writecb.aio_buf = send_data;
    ret = aio_write( &writecb );
    if ( ret ==  0 ) {
      while ( aio_error( &writecb ) == EINPROGRESS ) ;
      sent_len =  aio_return( &writecb );
error(" wanted to send %d actual sent %d %s", send_len, sent_len, strerror( errno )  );
    } else {
      int err = errno;
