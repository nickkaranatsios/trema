...
_request_send_group_stats( const struct ofp_group_stats_request *req, const uint32_t transaction_id ) {
  ...
  OFDPE ret;
  if (  ( ret = get_group_stats( req->group_id, &stats, &nr_group_stats ) ) == OFDPE_SUCCESS ) {
...
static list_element *
_request_group_desc_stats( const uint32_t transaction_id ) {
  ...
  OFDPE ret;
  if ( ( ret = get_group_desc_stats( &stats, &group_desc_stats_nr ) ) == OFDPE_SUCCESS ) {
...
list_element * ( *request_group_desc_stats )( const uint32_t transaction_id ) = _request_group_desc_stats;
...
_request_send_table_features_stats( uint32_t transaction_id ) {
  ...
  OFDPE ret;
  for ( uint8_t i = 0; i <= FLOW_TABLE_ID_MAX; i++ ) {
    if ( ( ret = get_flow_table_features( i, &table_features ) ) == OFDPE_SUCCESS ) {
      ...
      uint16_t type = OFPET_BAD_REQUEST;
      uint16_t code = OFPBRC_BAD_TABLE_ID;
      get_ofp_error( ret, &type, &code );
      send_error_message( transaction_id, type, code );
    }
  }
