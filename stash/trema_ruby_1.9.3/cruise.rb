...
def run_unit_test
  test "Running unit tests ..." do
    sh "rake -f test_rakefile unittests"
    sh "rake -f test_rakefile"
    sh "rake -f Rakefile spec"
  end
  measure_coverage
end
...
def init_cruise
  $start_time = Time.now
  sh "rake -f test_rakefile distclean"
  sh "bundle install"
  mkdir_p Trema.log
  mkdir_p Trema.pid
  mkdir_p Trema.sock
